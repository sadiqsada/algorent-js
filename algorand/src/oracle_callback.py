...
def approval_program():
    ...
    settlement_status = Btoi(Txn.application_args[2])  # Value must be provided in micro algo (i.e 1 is 0.000001)
    title_signature = Txn.application_args[1]
    get_settlement_status_callback = Seq(
        [
            Assert(
                And(
                    is_whitelisted,
            ...
            App.globalPut(title_signature, settlement_status),
    ...
    program = Cond(
        ...
        [Txn.application_args[0] == Bytes("get_settlement_status_callback"), get_settlement_status_callback]
    )
    return program
